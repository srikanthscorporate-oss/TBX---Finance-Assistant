// G20: the chart set exists, uses only palette tokens, and its pages and endpoints are in place.
...
for (const c of ['export function BarSeries', 'export function LineSeries', 'export function RingChart',
  ...
  if (!charts.includes(c)) fail('G20', `charts.tsx lacks ${c}`);
const hexes = charts.match(/#[0-9a-fA-F]{6}\b/g) || [];
if (hexes.length) fail('G20', `raw hex colours in charts.tsx: ${[...new Set(hexes)].join(', ')}`);
if (!/var\(--seq-|var\(--cat-|var\(--good\)|var\(--critical\)/.test(charts)) fail('G20', 'charts do not reference palette tokens');
const run = fs.readFileSync(path.join(ROOT, 'apps/web/components/RunPane.tsx'), 'utf8');
for (const c of ['TimingBar', 'SignalDots', 'RingChart']) if (!run.includes(c)) fail('G20', `RunPane does not use ${c}`);
const obs = fs.readFileSync(path.join(ROOT, 'apps/web/components/Observability.tsx'), 'utf8');
for (const c of ['Sparkline', 'TimingBar', 'BarSeries']) if (!obs.includes(c)) fail('G20', `Observability does not use ${c}`);
...
if (u.runs > 0) {
  if (!u.time_split_ms || typeof u.time_split_ms.llm !== 'number') fail('G20', 'usage lacks time_split_ms');
  if (!Array.isArray(u.recent) || !u.recent.length) fail('G20', 'usage lacks recent history');
  const r = u.recent.at(-1);
  for (const k of ['state', 'duration_ms', 'tokens', 'at']) if (!(k in r)) fail('G20', `recent run lacks ${k}`);
  if (JSON.stringify(u.recent).match(/counterparty|transaction_id|utr|account_number|₹/)) fail('G20', 'recent history leaks financial data');
}
pass('G20', 'six chart primitives, token-only colour', 'run pane + observability wired',
